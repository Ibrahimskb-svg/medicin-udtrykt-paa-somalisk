...
// ── Human Inhaler Animation — realistic person demonstrating technique ──────
const TWIST_LABEL = { da:"↺ DREJ", en:"↺ TURN", so:"↺ LEEXI", ar:"↺ أدر" };

function HumanInhalerSVG({ step, type = "ventoline", language = "da" }) {
  ...
  const S = "#FDBCB4"; const SD = "#E8967A"; // skin
  const H = "#3A2210";                        // hair
  const SHIRT = "#2563EB";
  const PANTS = "#1E3A5F";
  ...
  return (
    <div style={{ width: 280, height: 330, margin: "0 auto", position: "relative" }}>
      <style>{`
        ...
        @keyframes hmGlow {
          0%,100% { opacity:0.62; r:2.6; }
          50%     { opacity:0.88; r:3.4; }
        }
      ...
      <svg viewBox="0 0 280 330" width="280" height="330">
        ...
        {/* ─── EXHALE PARTICLES (flow out left/up from mouth) ─── */}
        ...
        <g style={chestStyle}>
          ...
          {/* Medicine micro-particles inside lungs */}
        ...
        {/* Breath-hold pulse ring around chest */}
        {isHolding && <>
        ...
        {type === "ventoline" && (inhalerAtMouth ? (
          /* At mouth — coming from right, mouthpiece touching lips */
          <g transform="translate(148, 56)">
            ...
            {/* Mouthpiece pointing toward face (left) */}
            ...
            {/* Spray particles going into mouth */}
        ...
        {/* ─── WATER GLASS (Symbicort rinse step) ─── */}
        {isRinsing && (
...
function VentolineSVG({ step }) {
  return <HumanInhalerSVG step={step} type="ventoline" language="da" />;
}
function SymbicortSVG({ step, language = "da" }) {
  return <HumanInhalerSVG step={step} type="symbicort" language={language} />;
}
        ...
        <div style={{ padding:"20px 20px 32px" }}>
          ...
          <div style={{ background:"#fff",borderRadius:"20px",border:`1.5px solid ${theme.border}`,padding:"20px",marginBottom:"20px",boxShadow:`0 4px 16px ${theme.primary}10` }}>
            ...
            <div style={{ marginBottom:"16px" }}>
              <HumanInhalerSVG step={activeStep} type={inhaler} language={language} />
            </div>
